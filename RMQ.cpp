...
int RMQ::rangeMin(int i, int j)
{
    ...
    return seq[etSeq[resIndex]];
}

int RMQ::singleBlockRMQ(int block, int i, int j)
{
...
*/
int RMQ::blockRangeRMQ(int k, int l)
{
...
int RMQ::minByDepth(int i, int j)
{
...
void RMQ::eulerTour()
{
    ...
    firstOccurrence.assign(seq.size(), -1);
    etSeq.clear();
...
void RMQ::buildCartesianTree()
{
    ...
    leftChild.assign(seq.size(), -1);
    rightChild.assign(seq.size(), -1);
    std::stack<int> s;
